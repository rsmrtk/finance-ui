...
export function CategoriesPage() {
  ...
  const { data: categories = [], isPending: catPending } = useQuery({
    queryKey: ["categories"],
  ...
  const sections: {
    ...
    labelKey: "cat.expense" | "cat.income";
  }[] = [
  ...
  return (
    <div className="max-w-6xl flex flex-col gap-6">
      ...
      <AnimatePresence>
        {addType && (
          <AddCategoryModal type={addType} onClose={() => setAddType(null)} />
        )}
      </AnimatePresence>
    </div>
  );
      ...
      >
        ...
        <div>
          <div className="flex items-center justify-between mb-2">
            ...
            <div
              className="relative flex rounded-lg overflow-hidden border text-xs"
            ...
            {CATEGORY_COLORS.map((color) => (
              <motion.button
                ...
                style={{
                  background: `#${color}`,
                  outline:
                    color === colorHex ? "2px solid var(--text)" : "none",
                  outlineOffset: 3,
                }}
              />
            ))}
